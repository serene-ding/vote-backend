...
app.post("/vote", (req, res, next) => {
  ...
  if (userId != undefined) {
    ...
    var stmt2 = db.prepare(
      "INSERT INTO options (voteId, content) VALUES (?,?)"
    );
...
app.get("/vote/:voteId", (req, res, next) => {
  ...
  if (vote) {
    ...
    if (false && vote.anonymous && req.loginUser.userId != vote.userId) {
  ...
  if (vote) {
    ...
    if (voteWsMap[voteId]) {
      voteWsMap[voteId].forEach((ws) => {
        ...
        if (false && vote.anonymous && userId !== vote.userId) {
          cloned.forEach((it) => {
            if (it.userId != userId) {
              it.userId = "秘";
              it.avatar = "秘";
...
app.post("/upload", upload.any(), (req, res, next) => {
  var files = req.files;
  console.log(files);
  var urls = files.map((file) => `/uploads/` + file.originalname);
  res.json(urls);
});
